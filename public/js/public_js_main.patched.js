...
import { hideStatusMessages, showStatusMessage, updateUrlStatus, displayResults, updateApiParams, showNearestLocationModal } from './ui.no-media.js';
...
export const dom = {
    ...
    fromBeginCursorCheckbox: document.getElementById('fromBeginCursor'),
    fromLastCursorCheckbox: document.getElementById('fromLastCursor')
};
// --- Cursor checkboxes exclusivity ---
try {
    dom.fromBeginCursorCheckbox?.addEventListener('change', () => {
        if (dom.fromBeginCursorCheckbox.checked) dom.fromLastCursorCheckbox.checked = false;
    });
    dom.fromLastCursorCheckbox?.addEventListener('change', () => {
        if (dom.fromLastCursorCheckbox.checked) dom.fromBeginCursorCheckbox.checked = false;
    });
...
export const state = {
    allResults: [],
    isFetching: false,
    clientId: `client_4y1s3w8d7_100005146594548`,
    // clientId: `client_pufn4z6zw_100005146594548`,
    // clientId: `client_yp2rhpgvv_100005146594548`,
    defaultApiParams: {
...
async function makeApiCallMultiUrl() {
    ...
    let shuffledUrlList = (apiName === 'get_list_ig_user_stories') ? shuffleArray([...urlList]) : urlList;
...
for (let i = 0; i < shuffledUrlList.length; i++) {
        ...
        let cursorToUse = "";
        const forceBegin = !!dom.fromBeginCursorCheckbox?.checked;
        const forceLast  = !!dom.fromLastCursorCheckbox?.checked;
        ...
        if (apiName === 'get_list_fb_user_photos' || apiName === 'get_list_ig_post') {
            ...
            if (forceBegin) {
                ...
                // Default behavior: ask user via modal
